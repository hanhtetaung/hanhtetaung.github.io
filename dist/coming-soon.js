function h(r,o){if(r===void 0){if(o===Array)return[];if(o===Object)return{};if(o===Boolean)return!1;if(o===Number)return 0;return""}if(o===Boolean)return Boolean(r);if(o===Number)return Number(r);return r}function a(r,{props:o={},attrs:l=[],styles:d="",template:n}){let m=["props",...l];class c extends HTMLElement{static observedAttributes=m;constructor(){super();this.attachShadow({mode:"open"})}connectedCallback(){this._render()}attributeChangedCallback(){if(this.isConnected)this._render()}_getProps(){let t={},i=this.getAttribute("props");if(i)try{t=JSON.parse(i)}catch{t={}}let p={};for(let s of Object.keys(o))p[s]=h(t[s],o[s]);return p}_applyForwardedAttrs(){if(l.length===0)return;let t=this.shadowRoot.querySelector("[data-forward]");if(!t)return;for(let i of l)if(this.hasAttribute(i))t.setAttribute(i,this.getAttribute(i));else t.removeAttribute(i)}_render(){let t=this._getProps(),i=typeof n==="function"?n(t):n;this.shadowRoot.innerHTML=`<style>${d}</style>${i}`,this._applyForwardedAttrs()}}customElements.define(r,c)}var g=`
  :host {
...
`;a("app-link",{attrs:f,styles:g,template:v});var u=`
  :host {
    display: block;
  }
...
`,b=["width","height","src","alt"],y=()=>"<img data-forward />";a("app-logo",{attrs:b,styles:u,template:y});var e="769px",B="992px";var k=`
  :host {
    display: block;
  }
...
`;a("section-footer",{styles:H,template:_});var C=`
  :host {
    --local-width: 5rem;
    --local-heigth: auto;
...
`,E=["width","height","src","alt"],N=()=>"<img data-forward />";a("app-image",{attrs:E,styles:C,template:N});var O=`
  :host {
    --local-width: 3rem;
    --local-height: auto;
...
`,P=["src","alt"],S=()=>'<img data-forward width="200" height="200"/>';a("app-icon",{attrs:P,styles:O,template:S});var L=`
   section {
        margin-inline: auto;
        width: 80%;
        display: flex;
...
`;a("section-coming-soon-intro",{styles:L,template:T});
